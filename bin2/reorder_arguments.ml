...
let main filepath =
  ...
  let hes_original = Muhfl.parse filepath in
  let () =
    let path2' = filepath ^ "_reorder_orig.in" in
    ignore @@ Muhfl.Manipulate.Print_syntax.MachineReadable.save_hes_to_file ~file:path2' ~without_id:true true hes_original;
    print_endline @@ "Before Reordered: saved in " ^ path2' in
  let hes = Muhfl.reorder_arguments hes_original true false in
